	        	...
	        	if (isset($_FILES['image']) ) {
				 // files handle
				    ...
				    if ($extension =="jpg" || $extension =="png" || $extension =="jpeg"){
				    	...
				    	$iquery="INSERT INTO `restaurant_info`(`restaurent_name`, `email`, `phone`, `address`, `location`, `logo`, `password`, `role`, `cost`, `cuisine`,`time`) 
			        		VALUES ('$fullname','$email','$phone', '$address','$area','$file_name','$password','$role', '$cost', '$cuisine', '$time');";
			        	if ($con->query($iquery) === TRUE) {
